...
class DiskBlocks():
  def __init__(self, args):
    ...
    # initialize XMLRPC client connection to raw block server
    if args.ns>=MIN_SERVERS and args.ns<MAX_SERVERS:
      global N
      N = args.ns
    else:
    ...
    def serverconnectionport(i):
      server_url = 'http://' + SERVER_ADDRESS + ':' + str(i)
      self.block_server = xmlrpc.client.ServerProxy(server_url, use_builtin_types=True)
      self.arry_server.append(self.block_server)
      return 0
      
    
    
    if args.port0:
      serverports.append(args.port0)
    if args.port1:
      serverports.append(args.port1)
    if args.port2:
      serverports.append(args.port2)
    if args.port3:
      serverports.append(args.port3)
    if args.port4:
      serverports.append(args.port4)
    if args.port5:
      serverports.append(args.port5)
    if args.port6:
      serverports.append(args.port6)
    if args.port7:
      serverports.append(args.port7)
    
    if N != len(serverports):
      print('ns and number of ports entered must match')    
      quit()  
    else:
      for i in serverports:
        serverconnectionport(i)   
  ...
  def HandleFSConstants(self, args):
    ...
    #blocks in each server
    SERVER_NB=TOTAL_NUM_BLOCKS//N

## Put: interface to write a raw block of data to the block indexed by block number
## Blocks are padded with zeroes up to BLOCK_SIZE

  # byteXor to help in fault tolerance: repair procedure

  def byteXor(self, input1, input2,input3):
    Value=[] #input1=[1,0,1,0]
    input1=bytes(input1)
    input2=bytes(input2)
    input3=bytes(input3)
    #print(input1)
    Value=bytearray([i^j^k for i,j,k in zip(bytes(input1),bytes(input2),bytes(input3))])
    return Value
    
  def byteXor2(self, input1, input2):
    Value=[] #input1=[1,0,1,0]
    input1=bytes(input1)
    input2=bytes(input2)
    #print(input1)
    Value=bytearray([i^j for i,j in zip(bytes(input1),bytes(input2))])
    return Value
  
  
  def Repair(self,server_index):
    for i in range(TOTAL_NUM_BLOCKS//(N-1)):
      #print(i)
      data=self.RepairBlock(server_index,i)
      ret = self.arry_server[server_index].Put(i,data)
    print("successful")
    
  
  
  def Mapping_VB_To_PB(self, block_number):
    Physical_Block_Number = block_number//(N-1)
  ...
  def RepairBlock(self, server_index, block_number):
    temp_block=[]
    for i in range(0,N):
      if i != server_index:
        temp_block.append(bytearray(self.arry_server[i].Get(block_number)))
    temp=bytearray(BLOCK_SIZE)
    #print("following is temp_block ",temp_block)
    for j in range(0,len(temp_block)):
      temp=self.byteXor2(temp_block[j],temp)
      #print(temp_block)
  ...
  def ParityCalculation(self, ServerID, Physical_block_number,parity_SID,block_data):
    blk=[]
    try:
      old=self.arry_server[ServerID].Get(Physical_block_number)
      if old=="error":
        old=self.RepairBlock(ServerID,Physical_block_number)
    except ConnectionRefusedError:
      old=self.RepairBlock(ServerID,Physical_block_number)
    try:
      old_p=self.arry_server[parity_SID].Get(Physical_block_number)
      if old_p=="error":
        old_p=self.RepairBlock(parity_SID,Physical_block_number)
    except ConnectionRefusedError:
      old_p=old_p=self.RepairBlock(parity_SID,Physical_block_number)
    blk.append(bytearray(block_data.ljust(BLOCK_SIZE,b'\x00')))
    #print("old data      :",old)
    blk.append(old)
    blk.append(old_p)
    blk.append(bytearray(BLOCK_SIZE))
    blk[3]=self.byteXor(blk[0],blk[1],blk[2])
    #for i in range(0,len(blk)-1):
      #blk[3]=self.byteXor(blk[0],blk[1],blk[2])
      #for k in range(len(blk[0])):
        #blk[3][k]=self.byteXor(blk[0][k],blk[1][k],blk[2][k])
   
    
    #for i in range(0,len(blk)-1):
      #blk[i+1]=self.byteXor(blk[i],blk[i+1])
    return blk[3]
    
 
  def Put(self, block_number, block_data):
    
    ServerID, physical_block_number, parity_SID= self.Mapping_VB_To_PB(block_number)
    #logging.info("serverid:"+str(ServerID)+" phys: "+str(physical_block_number))
    
    parity=self.ParityCalculation(ServerID,physical_block_number,parity_SID,block_data) 
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS): 
      ...
      #Storing the block contents and parity at respective locations
      
      if ServerID in range(0,len(self.arry_server)):
      	try:
      	  self.arry_server[parity_SID].Put(physical_block_number,parity)
      	except ConnectionRefusedError:
      	  pass
      	try:
      	  ret = self.arry_server[ServerID].Put(physical_block_number,putdata)
      	#self.arry_server[parity_SID].Put(physical,block_number,parity)
      	  if ret == -1:
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def ServerGet(self, block_number):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      if ServerID in range(0,len(self.arry_server)):
        try:
          data = self.arry_server[ServerID].Get(physical_block_number)
          #print(data)
          #RepairCorruptedBlock
          if data=="error":
        #if data==bytearray(bytearray('Checksum did not match','utf-8').ljust(BLOCK_SIZE,b'\x00')):
            logging.error('A Server block is corrupted')
            #quit()
            data=self.RepairBlock(ServerID,physical_block_number)
        except ConnectionRefusedError:
          data=self.RepairBlock(ServerID,physical_block_number)
...
## RSM: read and set memory equivalent
  ...
  def RSM(self, block_number):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      if ServerID in range(0,len(self.arry_server)):
      	data = self.arry_server[ServerID].RSM(physical_block_number)
      return bytearray(data)
